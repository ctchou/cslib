...
theorem IsRegular.regex [Inhabited Symbol]
    (r : RegularExpression Symbol) : r.matches'.IsRegular := by
  induction r with
  | zero => simp
  | epsilon => simp
  | char a => simp [IsRegular.char]
  | plus P Q => grind [RegularExpression.matches', IsRegular.add]
  | comp P Q => grind [RegularExpression.matches', IsRegular.mul]
  | star P => grind [RegularExpression.matches', IsRegular.kstar]
